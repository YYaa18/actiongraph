...
The module catalog is checked against `settings.gradle.kts`, the BOM, and each module's direct Gradle project dependencies in tests. Catalog `requires`, `optionalWith`, and composition profile references must point to real catalog modules, and Spring Boot starter auto-configuration imports must match their annotated auto-configuration classes. A new module must be cataloged, classified, and either included in the BOM or explicitly treated as sample-only.
