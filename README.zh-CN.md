...
自研网关、endpoint adapter 或 Java 8 老系统如果需要复用控制层错误响应结构，或通过 HTTP 调用已部署的 ActionGraph Runtime API，可以直接依赖 `actiongraph-control-plane-api`。它提供统一的 `ControlPlaneErrorResponse(error, message)`，以及零外部依赖的 `ActionGraphRuntimeHttpClient`，可调用 `/interpret`、`/runs` 和 `/runs/{runId}/resume`；内置 Runtime、Component Catalog、Human Review、Callback 和 Console starter 会传递依赖它。

自研网关或 Spring MVC endpoint starter 如果只需要复用控制层共享密钥语义，可以直接依赖 `actiongraph-control-plane-auth`。它负责校验 Header 名、在未配置 `shared-secret` 时跳过 token 读取、使用常量时间比较 token；内置 Runtime、Component Catalog、Human Review、Callback 和 Console starter 会传递依赖它。它不是企业 IAM / RBAC 层，生产身份、权限和网关策略仍由接入系统或后续治理组件负责。
