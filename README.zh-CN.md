...
- **Human Review Spring Boot Starter**：仓储型审批策略和审批回调端点作为可选生态组件独立引入，供业务服务或外部审批系统使用。
...
- **Console Spring Boot Starter**：控制层/监管台作为可选生态组件独立引入，提供只读 Console 页面和查询端点，不强绑具体读模型。
...
| `actiongraph-governance` | 可选非 Spring 治理策略：脱敏、额度、多级审批路由、规则权限/租户校验 |
...
| `actiongraph-governance-spring-boot-starter` | 可选治理策略自动装配：脱敏、额度、多级审批路由 |
...
| `actiongraph-human-review-spring-boot-starter` | 可选仓储型审批策略与审批回调端点，供业务服务或外部审批系统使用 |
...
非 Spring 服务如果只需要框架内置的脱敏、额度、多级审批路由或规则权限/租户校验，可以直接依赖 `actiongraph-governance`，手工组装对应策略，不需要引入 Spring starter。
...
非 Spring 服务如果需要审批任务仓储、审批链或审批回调处理，可以直接依赖 `actiongraph-human-review`，不需要引入 Spring MVC endpoint。

引入 `actiongraph-governance-spring-boot-starter` 后，`actiongraph.masking.*`、`actiongraph.limits.*` 与 `actiongraph.human-review.risk-based-approval-chain` 才会生效。只引入基础 Spring starter 时，这些治理能力保持中性默认值：不脱敏、默认权限放行、无额度升级，高风险动作使用安全挂起默认策略。

引入 `actiongraph-human-review-spring-boot-starter` 后，Spring 服务会获得仓储型人工审批默认装配。启用 `actiongraph.human-review.callback-endpoint.enabled=true` 后，审批系统回调可直接 POST 到配置的端点。该 starter 默认提供内存 `HumanReviewRepository`，同时启用 `actiongraph-human-review-jdbc-spring-boot-starter` 后会使用持久化实现。
...
自研控制台、CLI 或企业网关适配可以直接依赖 `actiongraph-console-core`，复用只读查询服务、JSON 响应模型和 `ConsoleRunRepository` 端口，不引入 Spring MVC endpoint，也不强绑 JDBC。需要直接读取 ActionGraph trace 表时，再引入 `actiongraph-console-jdbc`。Spring MVC 控制层引入 `actiongraph-console-spring-boot-starter` 暴露页面和查询端点，并提供任意 `ConsoleRunRepository` Bean；如果希望仓储从 `DataSource` 自动创建，再叠加 `actiongraph-console-jdbc-spring-boot-starter`。启用 `actiongraph.console.enabled=true` 后，会暴露只读运行监控接口：
