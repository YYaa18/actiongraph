...
- **Control Plane Spring Boot Starter**：运行入口、审批任务、审批回调、Console API/UI/导出端点可通过一个可选聚合包组合使用；拆分 starter 仍可单独选择。
...
| `actiongraph-control-plane-spring-boot-starter` | 可选控制面聚合包，组合 Runtime / Human Review / Callback / Console 端点 |
...
如果单个 Spring MVC 部署需要同时暴露运行入口、审批任务、审批回调和 Console API/UI/导出端点，可以引入 `actiongraph-control-plane-spring-boot-starter`。它只是拆分 endpoint starter 的聚合坐标，不创建业务 Action、解释器、仓储、JDBC adapter、LLM client 或治理策略。
