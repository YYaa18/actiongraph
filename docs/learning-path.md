...
| 阶段 | 阅读入口 | 到达结果 |
|---|---|---|
| L0 Hello Agent / Schema-first Goal | `quick-start.html#l0`, `frameworkization/golden-path.md` | 一个注解类、一个 schema record、一个 `ActionGraph.start(...)` 调用 |
...
| L3 配置即能力 | `frameworkization/goal-configuration.md`, `frameworkization/goal-studio.md` | 配置 Goal、测试环境起草、bundle 指纹导入、生产侧重验可达性 |
| L4 生产化 | `quick-start.html#l3`, `frameworkization/jdbc-persistence.md`, `frameworkization/observability-spi.md`, `frameworkization/public-api-compatibility.md` | JDBC Trace/挂起快照、脱敏、指标、公共 API 兼容性纪律 |
| L5 跨服务 | `quick-start.html#l4`, `frameworkization/runtime-api.md`, `frameworkization/runtime-invocation-spi.md`, `frameworkization/control-plane-api.md` | HTTP 网关、Java 8 客户端、回调和外部事件边界 |
