...
For Spring applications, add `actiongraph-spring-boot-starter` and enable `actiongraph.human-review.callback-endpoint.enabled=true`; the `runId`, `actionId`, `expectedStageIndex`, `decision`, `reviewer`, and `comment` fields map to the callback endpoint at `/actiongraph/human-review/callbacks`. Send the shared secret through the configured token header, not as a JSON body field.
