...
For Spring applications, the `runId`, `actionId`, `expectedStageIndex`, `decision`, `reviewer`, and `comment` fields map to the starter endpoint at `/actiongraph/human-review/callbacks`. Send the shared secret through the configured token header, not as a JSON body field.
