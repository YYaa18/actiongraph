...
- **Control Plane API**：控制层错误响应契约抽成纯 Java 组件，内置 endpoint 和自研网关可复用统一的 `error/message` 结构。
...
| `actiongraph-control-plane-api` | 可选控制层错误响应契约组件 |
...
自研网关或 endpoint adapter 如果只需要复用控制层错误响应结构，可以直接依赖 `actiongraph-control-plane-api`。它提供统一的 `ControlPlaneErrorResponse(error, message)`，内置 Runtime、Component Catalog、Human Review、Callback、Console API 和 Console Export 端点会传递依赖它。
