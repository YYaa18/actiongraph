...
- **Component Catalog**：框架模块、能力标签、依赖提示和推荐组合方案可作为机器可读目录独立引入，方便企业网关、控制台和部署检查复用。
- **Component Catalog Spring Boot Starter**：组件目录只读 HTTP 端点独立成可选控制层组件，可单独启用或通过控制面聚合包组合。
- **Control Plane API**：控制层响应契约与轻量 Runtime HTTP client 抽成 Java 8 兼容纯 Java 组件，老系统可不引入 Spring 直接调用已部署的 ActionGraph Runtime API。
- **Control Plane Auth**：控制层共享密钥 token 校验抽成纯 Java 组件，Runtime / Component Catalog / Human Review / Callback / Console endpoint starter 复用同一套语义。
...
| `actiongraph-component-catalog` | 可选机器可读组件目录与组合方案元数据 |
| `actiongraph-control-plane-api` | Java 8 兼容的控制层响应契约与轻量 Runtime HTTP 客户端 |
| `actiongraph-control-plane-auth` | 可选控制层共享密钥 token 校验组件 |
...
非 Spring 服务、CLI、企业网关或部署检查如果需要结构化读取 ActionGraph 的模块、能力标签、依赖提示和推荐组合方案，可以直接依赖 `actiongraph-component-catalog`。Spring MVC 控制层需要只读目录端点时，再引入 `actiongraph-component-catalog-spring-boot-starter` 并启用 `actiongraph.component-catalog.enabled=true`：
...
自研网关、endpoint adapter 或 Java 8 老系统如果需要复用控制层错误响应结构，或通过 HTTP 调用已部署的 ActionGraph Runtime API，可以直接依赖 `actiongraph-control-plane-api`。它提供统一的 `ControlPlaneErrorResponse(error, message)`，以及零外部依赖的 `ActionGraphRuntimeHttpClient`，可调用 `/interpret`、`/runs` 和 `/runs/{runId}/resume`；内置 Runtime、Component Catalog、Human Review、Callback、Console API 和 Console Export 端点会传递依赖它。

自研网关或 Spring MVC endpoint starter 如果只需要复用控制层共享密钥语义，可以直接依赖 `actiongraph-control-plane-auth`。它负责校验 Header 名、在未配置 `shared-secret` 时跳过 token 读取、使用常量时间比较 token；内置 Runtime、Component Catalog、Human Review、Callback、Console API 和 Console Export 端点会传递依赖它。它不是企业 IAM / RBAC 层，生产身份、权限和网关策略仍由接入系统或后续治理组件负责。
...
如果单个 Spring MVC 部署需要同时暴露运行入口、组件目录、审批任务、审批回调和 Console API/UI/导出端点，可以引入 `actiongraph-control-plane-spring-boot-starter`。它只是拆分 endpoint starter 的聚合坐标，不创建业务 Action、解释器、仓储、JDBC adapter、LLM client 或治理策略。
...
- [控制层共享认证](docs/frameworkization/control-plane-auth.md)
