...
- **Public API 快照守门**：CI 会比对已发布模块的 public/protected API surface，避免兼容性变化静悄悄进入主干。
- **JSpecify 空安全合同**：已发布 API 包默认 `@NullMarked`，可选输入和可空 accessor 用 `@Nullable` 明确标注，方便 IDE、Kotlin 和后续静态检查接入。
- **架构边界守卫测试**：发布模块依赖方向、core 纯净性、Spring 隔离和样例代码反向依赖都由测试固定。
- **API 稳定性标注**：用 `@Experimental` / `@Internal` 区分可早期采用的演进接口和不建议业务直接依赖的内部实现面。
...
- **ActionGraph 正门与 Runtime SPI**：业务代码优先注入根门面 `ActionGraph` 调用 `start` / `chat` / `resume`；GoalCatalog、GoalInterpreter、解释结果、Blackboard seeder 与 `ActionGraphRuntimeOperations` 仍可作为控制面和自定义适配层复用。
...
| `actiongraph-core` | 核心 Action、注解适配器、Planning、Runtime、Policy、Trace、Observability SPI、JSpecify 空安全合同、公开合同 Javadoc、SLF4J API 诊断日志、目标解释契约、运行入口与结构化记忆 API |
| `actiongraph-control-plane-api` | Java 8 兼容的组件目录、JSpecify 空安全合同、控制层响应契约、`.properties` 聚合配置、安全 GET 重试、统一聚合 / Runtime / Component Catalog / Human Review / Console HTTP 客户端与共享密钥 token 校验 |
...
现代业务代码优先注入根门面 `ActionGraph`，调用 `start(goalType, parameters)`、`chat(input, knownParameters)` 或 `resume(runId)`。非 Spring 服务可以直接依赖 `actiongraph-core` 组装同一个门面，复用目标目录、规则解释器、注解 seeder 和确定性执行能力，不强制 HTTP 形态。`ActionGraphRuntimeOperations` 与 `ActionGraphRuntimeApiService` 继续作为控制面 HTTP 适配层，适用于自定义 Controller、MQ 消费者、ESB 适配器、Java 8 网关和请求 metadata 采集。Spring MVC 服务需要入口端点时，再引入 `actiongraph-spring-boot-starter` 并启用 `actiongraph.runtime.api.enabled=true`：
...
- [Public API 兼容性检查](docs/frameworkization/public-api-compatibility.md)
- [空安全合同](docs/frameworkization/null-safety.md)
- [架构边界守卫](docs/frameworkization/architecture-guardrails.md)
- [API 稳定性标注](docs/frameworkization/api-stability-annotations.md)
