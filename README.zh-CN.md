...
引入 `actiongraph-governance-spring-boot-starter` 后，`actiongraph.masking.*`、`actiongraph.limits.*` 与 `actiongraph.human-review.risk-based-approval-chain` 才会生效。只引入基础 Spring starter 时，这些治理能力保持中性默认值：不脱敏、默认权限放行、无额度升级、单级审批。
