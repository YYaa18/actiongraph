...
Spring 服务如果需要结构化长期记忆，可以显式引入 `actiongraph-memory-spring-boot-starter` 获取内存默认实现和 `MemoryContextLoader`；启用 JDBC starter 后会自动让位给 JDBC `MemoryRepository`。
...
引入 `actiongraph-human-review-spring-boot-starter` 后，Spring 服务会获得仓储型人工审批默认装配。启用 `actiongraph.human-review.callback-endpoint.enabled=true` 后，审批系统回调可直接 POST 到配置的端点。该 starter 默认提供内存 `HumanReviewRepository`，启用 JDBC starter 后会使用持久化实现。
...
自研控制台、CLI 或企业网关适配可以直接依赖 `actiongraph-console-core`，复用只读查询服务、JSON 响应模型和 `ConsoleRunRepository` 端口，不引入 Spring MVC endpoint，也不强绑 JDBC。需要直接读取 ActionGraph trace 表时，再引入 `actiongraph-console-jdbc`。引入 `actiongraph-console-spring-boot-starter` 并启用 `actiongraph.console.enabled=true` 后，如果应用提供 `DataSource`，Console starter 会暴露只读运行监控接口：
