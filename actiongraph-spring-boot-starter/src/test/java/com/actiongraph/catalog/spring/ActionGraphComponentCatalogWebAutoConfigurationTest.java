    ...
    void exposesModuleAndProfileLookupsThroughCustomPath() {
        contextRunner
                ...
                .run(context -> {
                    ...
                    mockMvc.perform(get("/internal/actiongraph/components/modules/actiongraph-spring-boot-starter"))
                            .andExpect(status().isOk())
                            .andExpect(jsonPath("$.kind").value("FRAMEWORK"))
                            .andExpect(jsonPath("$.compatibility").value("java21-plus"))
                            .andExpect(jsonPath("$.capabilities[*]", hasItem("runtime-http-api")));
