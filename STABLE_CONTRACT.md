...
- `com.actiongraph.runtime.api.batch`, `com.actiongraph.memory`, `com.actiongraph.llm`, `com.actiongraph.validation`, `com.actiongraph.graph`, `com.actiongraph.contribution`, retry/timeout execution policy additions, and `GoalDefinition.seedConditions` are currently experimental. They remain usable, documented, and tested, but japicmp and stable-contract review exclude them from 1.0 freezing until a future promotion decision;
...
- promoting or demoting `@Experimental` / `@Internal` API stability markers without documenting the compatibility impact;
