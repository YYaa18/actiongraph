...
引入 `actiongraph-human-review-spring-boot-starter` 并启用 `actiongraph.human-review.callback-endpoint.enabled=true` 后，审批系统回调可直接 POST 到配置的端点。该端点需要应用中存在 `HumanReviewRepository` Bean；`actiongraph-spring-boot-starter` 会提供内存默认实现，生产环境建议替换成 JDBC 持久化实现。
...
引入 `actiongraph-console-spring-boot-starter` 并启用 `actiongraph.console.enabled=true` 后，如果应用提供 `DataSource`，Console starter 会暴露只读运行监控接口：
