...
All built-in endpoint starters share `actiongraph-control-plane-api` for error response contracts, Java 8 compatible aggregate / properties-based aggregate / Runtime / Component Catalog / Human Review / Console HTTP client support, and shared-secret token checks. The API component keeps the JSON error shape, header lookup, disabled-secret semantics, and constant-time comparison consistent across Runtime API, Component Catalog, Human Review API, callback, and Console endpoints. The Spring aggregate starter exposes server-side endpoint modules; the Java 8 aggregate client and properties adapter are only caller-side conveniences for legacy systems. These are still lightweight control-plane utilities; enterprise identity, gateway policy, RBAC, tenant checks, and rate limits remain outside this aggregate.
