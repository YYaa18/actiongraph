...
| `actiongraph-core` | 核心 Action、注解适配器、Planning、Runtime、Policy、Trace、目标解释契约、运行入口与结构化记忆 API |
...
| `actiongraph-spring-boot-starter` | 主 Spring Boot 集成：注解扫描、运行时默认 Bean、JDBC 仓储、结构化记忆、仓储型人审、治理策略，以及按配置启用的运行入口/组件目录/人审/Console HTTP 端点 |
