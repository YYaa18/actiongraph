...
For Spring applications, add `actiongraph-human-review-spring-boot-starter`; the `runId`, `actionId`, `expectedStageIndex`, `decision`, `reviewer`, and `comment` fields map to its endpoint at `/actiongraph/human-review/callbacks`. Send the shared secret through the configured token header, not as a JSON body field.
