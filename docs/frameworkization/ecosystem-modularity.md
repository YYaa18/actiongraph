...
| Optional adapters | `actiongraph-annotations`, `actiongraph-memory-spring-boot-starter`, `actiongraph-human-review`, `actiongraph-human-review-spring-boot-starter`, `actiongraph-spring-boot-starter`, `actiongraph-governance`, `actiongraph-governance-spring-boot-starter`, `actiongraph-governance-human-review-spring-boot-starter`, `actiongraph-jdbc-spring-boot-starter`, `actiongraph-llm`, `actiongraph-llm-deepseek`, `actiongraph-persistence-jdbc` | Pure Java annotation action registration, Spring memory/JDBC wiring, repository-backed human review with task query APIs, Spring review-policy/JDBC wiring, Spring runtime wiring, reusable governance policies, Spring governance wiring, Spring core JDBC wiring, provider-neutral LLM goal interpretation, DeepSeek provider adapter, low-level durable repositories |
...
- JDBC review task storage adds `actiongraph-persistence-jdbc`; Spring Boot JDBC review task storage is wired by `actiongraph-human-review-spring-boot-starter` when `actiongraph.persistence.jdbc.enabled=true`.
...
- Durable Spring Boot production runs add `actiongraph-jdbc-spring-boot-starter`; non-Spring/manual runtimes add `actiongraph-persistence-jdbc`. Runtime, memory, and review-task durability now share this one low-level JDBC module.
...
- Single-deployment built-in control planes should explicitly choose the runtime, component-catalog, human-review, callback, and Console endpoint starters they expose.
...
Core provides structured memory records, the repository contract, the in-memory repository, and the Blackboard context loader. These types can be used without Spring, JDBC, LLM, governance, or console modules.

The Memory Spring Boot starter depends on `actiongraph-core` instead of a separate memory module. It provides Spring defaults for `MemoryRepository` and `MemoryContextLoader`; it also wires the JDBC memory repository when the shared JDBC persistence switch is enabled.
...
The full control-plane surface is composed explicitly from the Spring MVC endpoint starters for runtime entry, human-review task/callback APIs, component catalog, and the Console surface. Console read-model repository wiring remains property-gated by the Console starter.
...
Core maps durable business memory into typed Blackboard context. This capability must not depend on Spring, JDBC, LLM providers, governance, or endpoints.
...
`actiongraph-control-plane-api` is an optional ecosystem utility: it exposes Java 8 compatible response DTO contracts, a lightweight aggregate control-plane facade, properties-based aggregate configuration, safe GET retries, individual Runtime API / Component Catalog / Human Review / Console HTTP clients, and shared-secret token verification for control-plane adapters and legacy callers. It must not depend on Spring, runtime, persistence, LLM providers, governance, repositories, endpoint modules, or third-party HTTP/JSON libraries.
...
`actiongraph-governance` is an optional policy library: it provides reusable non-Spring masking, amount-limit, rule-based permission, review-attribute, and risk-based approval-routing implementations. It must not register actions, persist state, or expose endpoints.
...
`actiongraph-persistence-jdbc` is a low-level persistence library for trace, suspended runs, trace read models, structured memory, and human-review tasks. It is usable without Spring and is the dependency that specialized services can wire manually. It must not register Spring beans by itself.
...
There is no separate control-plane aggregate starter. The runtime entry, component catalog, human-review task/callback API, and Console endpoint starters must remain independently selectable and must not change any endpoint's existing opt-in property gates.
