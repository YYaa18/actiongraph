...
- **Spring Boot Starter**：业务类无需实现框架接口，通过注解即可注册 Action，并可按配置暴露人工审批回调端点和只读 Console 端点。
...
- **只读 Console Read Model**：JDBC Trace 表可查询最近运行摘要、终态/挂起状态和审计链验证结果，支撑后续服务化监管视图。
...
启用 `actiongraph.console.enabled=true` 后，如果应用同时引入 `actiongraph-persistence-jdbc` 并提供 `DataSource`，starter 会暴露只读运行监控接口：
...
返回内容包含 run 状态、首尾 Trace 时间、Trace 事件数、审计链是否完整以及首个断链序号。配置 `actiongraph.console.shared-secret` 后，访问方必须携带 Console token Header。
