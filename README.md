...
- API stability annotations for distinguishing stable, experimental, and internal public surfaces
...
Production systems should not invoke Gradle or sample CLI commands to start ActionGraph runs. Batch processing is application-owned: `BatchGoalInterpreter` is the framework SPI for token-efficient grouped interpretation, while structured source records should map directly to goals and known parameters when no LLM is needed. Model-specific batching, context-window chunking, JSON-mode details, and provider transport belong in a `BatchGoalInterpreter` or `LlmClient` adapter. Java 8 callers can depend on the `ActionGraphRuntimeGateway` interface and use `ActionGraphRuntimeHttpClient` as the default implementation against a deployed ActionGraph service.
...
Console API: GET /actiongraph/console/runs?limit=50&offset=0&status=COMPLETED&auditComplete=true
Console API: GET /actiongraph/console/runs/{runId}
Console API: GET /actiongraph/console/runs/{runId}/trace
Console API: GET /actiongraph/console/runs/export.csv
Console API: GET /actiongraph/console/runs/{runId}/trace/export.csv
Console API: GET /actiongraph/console/runs/{runId}/trace/export.jsonl
```
...
- [API stability annotations](docs/frameworkization/api-stability-annotations.md)
