...
- **Human Review API Spring Boot Starter**：审批任务列表、运行维度查询、详情、决策与审批回调 HTTP 端点独立成可选控制层组件。
- **控制面端点组合**：运行入口、组件目录、审批任务、审批回调、Console API/UI/导出端点通过显式 endpoint starter 组合使用，不再额外发布聚合包。
...
- **Runtime API**：目标解释、启动运行、恢复运行和请求元数据 Trace 记录入口服务独立成纯 Java 组件，企业网关、CLI 或自研控制器可直接复用。
- **Runtime API Spring Boot Starter**：解释目标、启动运行、恢复挂起运行和白名单请求头 Trace 记录的 HTTP 端点独立成可选控制层组件。
- **Component Catalog**：框架模块、能力标签、依赖提示和推荐组合方案可作为 Java 8 兼容机器可读目录独立引入，方便企业网关、控制台和部署检查复用。
- **Component Catalog Spring Boot Starter**：组件目录只读 HTTP 端点独立成可选控制层组件，可单独启用或与其他 endpoint starter 组合。
...
| `actiongraph-core` | 核心 Action、Planning、Runtime、Policy、Trace、目标解释契约、运行入口与结构化记忆 API |
...
| `actiongraph-control-plane-api` | Java 8 兼容的控制层响应契约、`.properties` 聚合配置、安全 GET 重试、统一聚合 / Runtime / Component Catalog / Human Review / Console HTTP 客户端与共享密钥 token 校验 |
...
非 Spring 服务如果只需要目标目录、规则解释器、解释结果到 Blackboard 的装配桥，或把“解释目标 → 装配 Blackboard → 启动运行 / 恢复运行”作为稳定入口服务，可以直接依赖 `actiongraph-core`。它只组合 `GoalInterpreter`、`GoalBlackboardSeederRegistry`、`GoapExecutor` 和 `ActionRegistry`，启动/恢复的 metadata 重载可把请求流水号、来源系统等写入 Trace，不自带 LLM provider、不创建持久化、不暴露 HTTP。Spring MVC 服务需要入口端点时，再引入 `actiongraph-runtime-api-spring-boot-starter` 并启用 `actiongraph.runtime.api.enabled=true`：
...
如果单个 Spring MVC 部署需要同时暴露运行入口、组件目录、审批任务、审批回调和 Console API/UI/导出端点，请显式引入 `actiongraph-runtime-api-spring-boot-starter`、`actiongraph-component-catalog-spring-boot-starter`、`actiongraph-human-review-api-spring-boot-starter` 和 `actiongraph-console-spring-boot-starter`。这些 endpoint starter 不创建业务 Action、解释器、运行期仓储、审批存储、LLM client 或治理策略；Console 读模型仓储仍由 Console starter 的配置开关控制。
