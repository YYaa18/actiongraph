...
- **Control Plane API**：控制层响应契约、统一聚合客户端、轻量 Runtime / Component Catalog / Human Review / Console HTTP client 与共享密钥 token 校验抽成 Java 8 兼容纯 Java 组件，老系统可不引入 Spring 直接调用已部署的 ActionGraph Runtime API、组件目录端点、审批任务端点和只读审计端点。
...
| `actiongraph-control-plane-api` | Java 8 兼容的控制层响应契约、统一聚合 / Runtime / Component Catalog / Human Review / Console HTTP 客户端与共享密钥 token 校验 |
